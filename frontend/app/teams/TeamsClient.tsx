...
export function TeamsClient({ teams: allTeams }: TeamsClientProps) {
  ...
  const teams = useMemo(() => {
    ...
    const maxSeason = Math.max(...allTeams.map(t => t.season));
    return allTeams.filter(team => team.season === maxSeason);
  }, [allTeams]);
