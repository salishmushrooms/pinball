...
class ScorePercentileReporterExcludeVenues:
    """Generate score vs percentile charts for pinball machines, excluding specific home venues."""

    def __init__(self, config_path: str, excluded_venues: list = None):
        """Initialize reporter with configuration file and venues to exclude."""
        self.config = self._load_config(config_path)

        # Set default excluded venues if not provided
        self.excluded_venues = excluded_venues if excluded_venues else ['T4B', 'IBX']

        # Load team-to-venue mapping
        self.team_home_venues = {}
        self.excluded_teams = set()
    ...
    def load_team_venue_mapping(self, data_path: str) -> None:
        """Load team to home venue mapping from teams.csv files."""
                ...
                with open(teams_file, 'r') as f:
                    ...
                    for row in reader:
                        if len(row) >= 3:
                            ...
                            self.team_home_venues[team_key] = venue_key

                            # Track teams whose home venue should be excluded
                            if venue_key in self.excluded_venues:
                                self.excluded_teams.add(team_key)

                print(f"Loaded team-venue mapping from season {season}: {len(self.team_home_venues)} teams")

            except FileNotFoundError:
                print(f"Warning: {teams_file} not found. Skipping team-venue mapping for season {season}.")

        if self.excluded_teams:
            excluded_venue_names = ', '.join(self.excluded_venues)
            print(f"Excluding home games for {len(self.excluded_teams)} teams with home venues: {excluded_venue_names}")
            print(f"  Excluded teams: {', '.join(sorted(self.excluded_teams))}")
    ...
    def should_exclude_score(self, team_key: str) -> bool:
        """Determine if a score from a specific team should be excluded."""
        return team_key in self.excluded_teams
    ...
    def extract_machine_scores(self, target_machine: str) -> list:
        """Extract scores for the specified machine from all matches, excluding scores from excluded teams."""
                ...
                for game in round_data.get('games', []):
                    ...
                    for pos in reliable_positions:
                        ...
                        if player_key and score_key in game and player_key in match_players:
                            ...
                            # Exclude scores from players on excluded teams
                            if self.should_exclude_score(player_info['team']):
        ...
        print(f"Extracted {len(machine_scores)} scores for machine '{target_machine}' (excluded {excluded_score_count} scores from filtered teams)")
    ...
    def generate_chart(self, target_machine: str, machine_scores: list, outlier_info: dict = {}, output_dir: str = "reports/charts") -> str:
        ...
        # Build title with exclusion info
        ...
        excluded_code = "_exclude_" + "_".join(self.excluded_venues)
        filename = f"{target_machine_normalized}_percentile_season_{seasons_code}_{venue_code}{ipr_code}{excluded_code}.png"
    ...
    def generate_report_file(self, target_machine: str, machine_scores: list, outlier_info: dict = {}) -> str:
        ...
        seasons_code = "-".join(self.seasons) if len(self.seasons) > 1 else self.seasons[0]
        excluded_code = "_exclude_" + "_".join(self.excluded_venues)
        output_file = f"reports/output/{target_machine_normalized}_percentile_season_{seasons_code}_{venue_code}{ipr_code}{excluded_code}.md"
...
def main():
    ...
    if len(sys.argv) < 2:
        print("Usage: python score_percentile_report_exclude_venues.py <config_file> [excluded_venue1,excluded_venue2,...]")
        print("\nExamples:")
        print("  python score_percentile_report_exclude_venues.py reports/configs/4bs_season22_all_machines.json")
        print("  python score_percentile_report_exclude_venues.py reports/configs/4bs_season22_all_machines.json T4B,IBX")
        print("\nDefault excluded venues: T4B (The 4 Bs), IBX (Ice Box)")
        sys.exit(1)

    config_file = sys.argv[1]

    # Parse excluded venues if provided
    excluded_venues = None
    if len(sys.argv) >= 3:
        excluded_venues = [v.strip() for v in sys.argv[2].split(',')]

    try:
        # Initialize reporter
        reporter = ScorePercentileReporterExcludeVenues(config_file, excluded_venues)
