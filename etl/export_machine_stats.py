...
def fetch_machine_scores(seasons: list[int], min_plays: int):
    ...
    filtered = {k: v for k, v in machines.items() if len(v["scores"]) >= min_plays}
...
def generate_score_interpretation(stats: dict) -> dict:
    ...
    if stats["skewness"] > 1:
        ...
        interpretation["skewness_description"] = (
            "Most scores cluster at lower values with some very high outliers"
        )
    elif stats["skewness"] > 0.5:
        interpretation["skewness_meaning"] = "moderately_right_skewed"
        interpretation["skewness_description"] = (
            "Scores tend toward lower values with a tail of higher scores"
        )
    elif stats["skewness"] < -0.5:
        ...
        interpretation["skewness_description"] = (
            "Scores are relatively evenly distributed around the median"
        )

    # Kurtosis interpretation
    if stats["kurtosis"] > 3:
        interpretation["kurtosis_meaning"] = "heavy_tailed"
        interpretation["kurtosis_description"] = (
            "More extreme scores (outliers) than a normal distribution"
        )
    ...
    if stats["cv"] > 1:
        ...
        interpretation["variability_description"] = (
            "Scores vary dramatically; high unpredictability"
        )
    elif stats["cv"] > 0.5:
